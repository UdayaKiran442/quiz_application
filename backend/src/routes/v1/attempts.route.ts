...
attemptsRouter.post('/add', async (c) => {
    try {
        ...
        return c.json({success: true, message: "Attempt added successfully", attemptId: attempt }, 200);
    } catch (error) {
        ...
        return c.json({ success: false, message: "Failed to add attempt", error: (error as Error).message }, 500);
    }
})
